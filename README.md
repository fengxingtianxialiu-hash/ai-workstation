...
- 👥 **多智能体协作**：支持流水线（顺序执行）和主从模式（指挥官分配任务），多 Agent 协同完成复杂任务，实时展示执行进度，支持查看完整中间结果并单独下载每个步骤的 Word 文档，**团队协作时自动检索知识库**
- 📚 **知识库 RAG**：支持上传 .txt/.md 文档，自动分块建立全文索引，Agent 可绑定知识库，对话和团队协作时自动检索注入相关知识，支持来源溯源和原文查看
