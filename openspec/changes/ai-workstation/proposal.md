...
构建一个基于 Vue 3 + Vite 的全功能 AI 工作台，部署在 GitHub Pages 上，提供对话、文件生成、Agent 管理等能力。目标是打造一个个人可用的、可跨端（H5 + 未来小程序）的 AI 工具平台，通过 Node.js 本地代理服务器解决跨域和协议适配问题，所有数据本地存储并支持一键导出/导入。
...
- 实现联网搜索功能：开关按钮、AI 查询重写、Bing 搜索、结果注入上下文
...
- 实现 Node.js 本地代理服务器：解决跨域、适配 Token/Plan 协议、流式内容提取（思考/正文分离）、联网搜索端点、图片生成端点
...
- `proxy-layer`: Node.js 本地代理服务器，跨域解决、协议适配、流式内容提取、联网搜索、图片生成
...
- **部署**: GitHub Pages (静态站点) + Node.js 本地代理服务器（开发环境）
